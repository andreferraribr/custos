...
gra <- pessoal %>% filter (`UORG Código` %in% descentralizadas) %>% group_by(UPAG) %>% summarise(custo_pessoal = as.integer (sum(`Custo Pessoal - Ativo`)), pessoal =  as.integer(round(sum(`Força de Trabalho`)/12)))

tabela(gra)


gra_clientes <- pessoal %>% filter (`UORG Código` %!in% descentralizadas) %>% group_by(UPAG, uf) %>% summarise(custo_pessoal = as.integer (sum(`Custo Pessoal - Ativo`)), pessoal =  as.integer(round(sum(`Força de Trabalho`)/12)))
