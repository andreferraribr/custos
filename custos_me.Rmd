...
selectInput("uf", label = h3("escolha o Estado:"),
    ...
    selected = "TODOS")
custeio_apoio <- custeio %>% filter(tipo_org == "apoio")
custo_pessoal_apoio <- custo_pessoal %>% filter(tipo_org == "apoio")

custo_total_reactive <- reactive(custo_total %>% filter( if_else( input$uf == "TODOS", uf != "TODOS" , uf == input$uf)))
custeio_reactive <- reactive(custeio_apoio %>% filter( if_else( input$uf == "TODOS", uf != "TODOS" , uf == input$uf)))
custo_pessoal_reactive <- reactive(custo_pessoal_apoio %>% filter( if_else( input$uf == "TODOS", uf != "TODOS" , uf == input$uf)))
```
...
renderValueBox({
 valueBox( reais(sum(custeio_reactive()$custeio)),
  color = "#f54242")
  })
```
...
renderValueBox({
 valueBox( reais(sum(custo_pessoal_reactive()$custo_pessoal)),
  color = "#f242f5")
  })
```
