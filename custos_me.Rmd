...
pessoal <- pessoal %>% mutate(forca_trabalho = round((`Força de Trabalho`)/12),3)
...
ggplot(custo_uds   , aes(x = reorder (uf,-(custeio+custo_pessoal)), y = (custeio+custo_pessoal), label= reais(custeio+custo_pessoal)))+ coord_flip() + 
...
### Ranking custeio administrativo (UDs)


```{r plot custeio UDs}
renderPlotly( ggplot(custo_uds   , aes(x = reorder (uf,-custeio), y = (custeio),label = reais(custeio)))+ coord_flip() + 
    geom_col(fill = if_else(custo_total$uf %in% input$ufs, "red", "gray"))+ theme_classic()+
...
### Ranking custo com pessoal (UDs)



```{r plot custo pessoal uds}
renderPlotly( ggplot(custo_uds   , aes(x = reorder (uf,-custo_pessoal), y = custo_pessoal, label = reais(custo_pessoal)))+ coord_flip() + 
    geom_col(fill = if_else(custo_total$uf %in% input$ufs, "red", "gray"))+ theme_classic()+
...
problema rateio. covid. custos fixos.
...
### custeio administrativo total (cliente + UDs)
...
renderPlotly(
  ...
  ggplot(custeio_adm_reactive()%>% filter (uf %in% input$ufs) %>% group_by(sub_tipo_custeio, tipo_org) %>% summarise(custeio = sum(custo))  , aes(x = reorder (sub_tipo_custeio,custeio), y = (custeio), label = reais(custeio)))+ coord_flip() + 
...
renderGauge({
max_value <- pessoal %>% filter (uf %in% input$ufs) %>% summarise(forca_trabalho = sum(forca_trabalho) )  
value <- pessoal %>% filter (uf %in% input$ufs, tipo_org == "UDs") %>% summarise(forca_trabalho = sum(forca_trabalho) ) 
gauge(sum(value$forca_trabalho), min = 0, max = sum(max_value$forca_trabalho), gaugeSectors(colors = "#fa0000"   )
)
})
...
Força de trabalho é igual ao valor arrendodado da quantidade de meses trabalhada dividido por 12.
Por exemplo: dois serviodres trabalharam durrante 12 meses em 2020 e um terceiro servidor trabalho por 9 meses. O total de meses trabalhado foi de Custeio administrativo de acordo com nota técnica 545845. rigidez. cedidos....
...
renderPlotly(
  ...
  ggplot(pessoal%>% filter (uf %in% input$ufs, tipo_org == "UDs",  `Grupo Situação Vínculo` == "ATIVO") %>% group_by( ocupacao) %>% summarise(custo_pessoal = sum(`Custo Pessoal - Ativo`), forca_trabalho = sum(forca_trabalho))  , aes(x = custo_pessoal, y = forca_trabalho, label = reais(custo_pessoal))) + 
...
renderDataTable(
tabela(pessoal %>%  filter (uf %in% input$ufs, tipo_org == "UDs", `Grupo Situação Vínculo` == "ATIVO") %>% group_by( ocupacao) %>% summarise(custo_pessoal = sum(`Custo Pessoal - Ativo`), forca_trabalho =  as.integer(  sum(forca_trabalho))) )
