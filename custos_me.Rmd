...
custos_reactive <- reactive(custos %>% filter(tipo_org == "apoio") %>% group_by(pi, ndd, centro_custo, uf) %>% summarise(custo = sum (custo)))

custeio <- (custos %>% group_by(uf, tipo_org)  %>% summarise(custeio  =  (sum(custo)/1000)))

pis <- (custos %>% filter(ugr_id %in% ugr_descentralizadas$ugr_id ,endsWith (pi_id, "UNIDES"), pi_id %!in% c( "S6403UNIDES","COVIDUNIDES" )) %>% group_by(pi_id) %>% summarise(custo = sum(custo)))
...
renderValueBox({
  ...
  teste <- custos_reactive() %>% group_by(uf,ndd) %>% summarise(custo = sum(custo))
  
 valueBox( reais(sum(teste %>% filter( if_else( input$uf == "TODOS", uf != "TODOS" , uf == input$uf))  )),
  color = "red")
  })
```
