...
renderValueBox({
 ...
 valueBox( custos %>% filter(uf %in% input$ufs) %>% summarise(custo = sum(custo)),
           color = "red")  
...
```{r}
renderPlotly( ggplot(custo_cliente    , aes(x = reorder (uf,-(custeio+custo_pessoal)), y = (custeio+custo_pessoal)))+ coord_flip() + 
    geom_col(fill = if_else(custo_total$uf == input$uf, "red", "gray"))+ theme_classic()
