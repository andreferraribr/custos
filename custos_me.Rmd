...
# importar relaçao das unidades descentralizadas (apoio)
...
# difenrenciar as unidades descentralizadas (apoio) dos clientes
...
* [Unidades Descentralizadas](https://www.gov.br/economia/pt-br/acesso-a-informacao/institucional/quem-e-quem/secretaria-executiva/secretaria-de-gestao-corporativa/unidades-descentralizadas)  
...
value <- custo_pessoal %>% filter (uf %in% input$ufs, tipo_org == "apoio") %>% summarise(custo_pessoal = sum(custo_pessoal) ) 
gauge(sum(value$custo_pessoal), min = 0, max = sum(max_value$custo_pessoal), gaugeSectors(colors = "#fa0000"   )
)
})
...
### custeio
```{r dados custeio}
renderDT(tabela(custos %>% select(uf, tipo_org, uo, ugr, ug_icc, pi, sub_tipo_custeio, ndd, centro_custo,custo)))
```
