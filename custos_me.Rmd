...
```{r}
renderPlotly( ggplot(custeio_adm%>% filter (uf %in% input$ufs) %>% group_by( tipo_org, sub_tipo_custeio) %>% summarise(custeio = sum(custo))  , aes(x = reorder (sub_tipo_custeio,custeio), y = custeio))+ coord_flip() + 
