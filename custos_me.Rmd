...
custeio <- (custos %>% group_by(uf, tipo_org) %>% filter (uf != "na")%>% summarise(custeio  =  (sum(custo)/1000)))
...
custo_total <- custo_total %>% filter (uf %!in% c("AC","RR","RO","AP", "DF") )

tabela (custo_total %>% group_by(uf, tipo_org) %>% summarise(custo_total  =  (sum(custo_pessoal+ custeio)))%>% pivot_wider( names_from = tipo_org, values_from = custo_total)  %>% mutate(percentual = apoio/cliente ))



```
```{r}
ggplot(custo_total %>% filter(tipo_org == "apoio") %>% pivot_longer(cols = c(custo_pessoal, custeio),names_to = "tipo_custo",values_to = "custo"), aes(x = uf, y = custo ,fill = tipo_custo)) + 
