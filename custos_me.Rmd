...
tabela(pessoal %>% group_by(uf, UPAG, tipo_org) %>% filter(uf %!in% c("AC","RR","RO","AP", "DF"))%>% summarise(custo_pessoal  =  (sum(`Custo Pessoal - Ativo`)/1000))%>% pivot_wider( names_from = tipo_org, values_from = custo_pessoal) %>% mutate(percentual = apoio/cliente ))
...
tabela(custos %>% group_by(uf,ug_emitente, tipo_org) %>% filter (uf != "na")%>% summarise(custo_apoios  =  (sum(custo)/1000)))


tabela(custos %>% group_by(uf,ug_emitente, tipo_org) %>% filter (uf != "na",uf %!in% c("AC","RR","RO","AP", "DF") )%>% summarise(custo_apoios  =  (sum(custo)/1000))%>% pivot_wider( names_from = tipo_org, values_from = custo_apoios)  %>% mutate(percentual = apoio/cliente ))
