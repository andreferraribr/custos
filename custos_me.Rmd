...
custo_pessoal <- (pessoal %>% group_by(uf,  tipo_org) %>% filter(uf %!in% c("AC","RR","RO","AP", "DF"))%>% summarise(custo_pessoal  =  (sum(`Custo Pessoal - Ativo`)/1000)))
...
custeio <- (custos %>% group_by(uf, tipo_org)  %>% filter(uf %!in% c("AC","RR","RO","AP", "DF", "na"))%>% summarise(custeio  =  (sum(custo)/1000)))
...
custeio_reactive <- reactive(custeio %>% filter( if_else( input$uf == "TODOS", uf != "TODOS" , uf == input$uf)))
custo_pessoal <- reactive(custo_pessoal %>% filter( if_else( input$uf == "TODOS", uf != "TODOS" , uf == input$uf)))
```
...
renderValueBox({
 valueBox( sum(custo_total_reactive()$apoio)/sum(custo_total_reactive()$cliente)*100)
  })
```
...
renderValueBox({
 valueBox( reais(sum(custo_total_reactive()$apoio)))
  })
```
...
renderValueBox({
 valueBox( reais(sum(custo_total_reactive()$cliente)))
  })
```
...
```{r}
renderPlotly( ggplot(custeio   , aes(x = reorder (uf,-custeio), y = custeio))+ coord_flip() + 
    geom_col())
```
...
```{r}
renderPlotly( ggplot(custo_total   , aes(x = reorder (uf,-apoio), y = apoio))+ coord_flip() + 
    geom_col())
```


### Ranking custeio  das unidades de apoio
